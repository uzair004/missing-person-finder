		...
		if (!errors.isEmpty()) {
			...
			if (req.file === undefined) {
				...
				if (faces.length === 0) {
					...
					faces.forEach((face) => {
						staticPool.workers.forEach((worker) =>
							worker.postMessage({
								toInsert: true,
...
router.get("/:id", function (req, res) {
	Person.findById(req.params.id)
		.then((foundPerson) => {
			User.findById(foundPerson.Author)
				.then((foundUser) => {
...
router.post("/person_search_by_image", upload, async function (req, res) {
	...
	try {
		...
		console.error(
			"error while executing workerFile func using staticPool.exec :",
			err
		);
	}
...
async function uploadImage(req, imgAsBase64, newPerson, res) {
	try {
		const result = await cloudinary.uploader.upload(imgAsBase64, {
			folder: personFolderPath,
			quality: "auto",
			width: 1024,
			crop: "limit",
		});
		...
		res.render("add_missing_person", {
			user: req.user,
			Photoerror:
				"Cannot upload image, possibly server problem, try again later",
		});
	}
}
...
async function incPersonCount(query, currentCount) {
	...
	User.updateOne(query, userUpdate).catch((err) =>
		console.error(`error while updating user person count`, err)
	);
}
...
function twofieldSearchAndShowRecords(
	searchField,
	searchTerm,
	searchField2,
	searchTerm2,
